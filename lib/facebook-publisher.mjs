function requireLiveCronPosts() {
  const flag = String(process.env.LIVE_CRON_POSTS || process.env.CRON_LIVE_POSTS || '0').trim().toLowerCase();
...
function getPageToken() {
  const fromEnv = (process.env.FACEBOOK_ACCESS_TOKEN || '').trim();
...
function getGroupId() {
  const fromEnv = process.env.FACEBOOK_GROUP_ID || '';
  if (fromEnv) return fromEnv;
  const fromMeta = safeMetaImport();
