// api/instagram.js
// Wave 3 activation: Instagram publisher endpoint
...
export default async function handler(req, res) {
  ...
  if (!checkDashboardApiKey(req)) {
    return res.status(401).json(buildEnvelope({ ok: false, action: 'instagram', status: 'error', error: 'Unauthorized' }));
  }

  if (req.method !== 'POST') {
    return res.status(405).json(buildEnvelope({ ok: false, action: 'instagram', status: 'error', error: 'Method not allowed - use POST' }));
  }
  ...
  try {
    ...
    const result = handleSocialPublisherRequest({
      ...
      payload: { ...body, publisherName: 'instagram' },
    });
    ...
    return res.status(500).json(buildEnvelope({
      ...
      action: 'instagram',
      status: 'error',
      error: 'Instagram publisher failed',
