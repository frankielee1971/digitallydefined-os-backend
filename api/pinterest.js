// api/pinterest.js
// Wave 5 activation: Pinterest publisher endpoint
...
export default async function handler(req, res) {
  ...
  if (!checkDashboardApiKey(req)) {
    return res.status(401).json(buildEnvelope({ ok: false, action: 'pinterest', status: 'error', error: 'Unauthorized' }));
  }

  if (req.method !== 'POST') {
    return res.status(405).json(buildEnvelope({ ok: false, action: 'pinterest', status: 'error', error: 'Method not allowed - use POST' }));
  }
  ...
  try {
    ...
    const result = handleSocialPublisherRequest({
      ...
      payload: { ...body, publisherName: 'pinterest' },
    });

    return res.status(200).json(result);
  } catch (err) {
    console.error('[pinterest] failed:', err);
    return res.status(500).json(buildEnvelope({
      ok: false,
      action: 'pinterest',
      status: 'error',
      error: 'Pinterest publisher failed',
