// api/facebook.js
// Wave 3 activation: Facebook publisher endpoint
...
export default async function handler(req, res) {
  ...
  if (!checkDashboardApiKey(req)) {
    return res.status(401).json(buildEnvelope({ ok: false, action: 'facebook', status: 'error', error: 'Unauthorized' }));
  }

  if (req.method !== 'POST') {
    return res.status(405).json(buildEnvelope({ ok: false, action: 'facebook', status: 'error', error: 'Method not allowed - use POST' }));
  }
  ...
  try {
    ...
    const result = handleSocialPublisherRequest({
      ...
      payload: { ...body, publisherName: 'facebook' },
    });
    ...
    return res.status(500).json(buildEnvelope({
      ...
      action: 'facebook',
      status: 'error',
      error: 'Facebook publisher failed',
