// api/tiktok.js
// Wave 5 activation: TikTok publisher endpoint
...
export default async function handler(req, res) {
  ...
  if (!checkDashboardApiKey(req)) {
    return res.status(401).json(buildEnvelope({ ok: false, action: 'tiktok', status: 'error', error: 'Unauthorized' }));
  }

  if (req.method !== 'POST') {
    return res.status(405).json(buildEnvelope({ ok: false, action: 'tiktok', status: 'error', error: 'Method not allowed - use POST' }));
  }
  ...
  try {
    ...
    const result = handleSocialPublisherRequest({
      ...
      payload: { ...body, publisherName: 'tiktok' },
    });
